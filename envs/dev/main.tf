...
locals {
  ...
  cors_validation_error = var.http_api_cors_allow_credentials && local.cors_has_wildcard ? (
    ...
    "For authenticated requests, you must specify explicit origins."
  ) : null
...
module "networking" {
  ...
  # Resilience: Keep true to avoid recreate errors if VPC already exists
...
module "ecr" {
  ...
  # Resilience: Keep true to skip creation if repo already exists
  create_if_not_exists = true
  aws_region           = var.aws_region
...
module "ecs_service" {
  ...
  environment_variables       = concat(
    ...
    # Add WebSocket notification broadcasting variables
    # Note: Table name follows pattern: {name}-websocket-connections
    # API endpoint will be constructed from websocket API Gateway (defined later in file)
    [
      {
...
resource "null_resource" "rds_disable_deletion_protection" {
  ...
  provisioner "local-exec" {
    ...
    command = <<-EOT
      set -e
      INSTANCE="shelfshack-dev-postgres"
      REGION="us-east-1"
  ...
  triggers = {
    ...
    rds_id      = "shelfshack-dev-postgres"  # Use fixed name to avoid dependency cycle
  }
}
...
module "websocket_lambda" {
  ...
  # Resilience: Keep true so module reuses existing resources when present
  create_if_not_exists = true
  aws_region           = var.aws_region
}
...
# This replaces the manually created API Gateway: shelfshack-backend-dev
...
# Resource destruction protection - only created when protection is enabled
# When allow_destruction=true, this resource doesn't exist, so no protection check happens
# When allow_destruction=false, this resource exists and will block destruction
resource "null_resource" "destroy_protection" {
  # Only create protection resource when destruction is NOT allowed
  count = local.can_destroy ? 0 : 1
  ...
  provisioner "local-exec" {
    ...
    command = <<-EOT
      ...
      echo "" >&2
      echo "To allow destruction, run:" >&2
      echo "  terraform destroy -var='allow_destruction=true' -var='db_master_password=YOUR_PASSWORD'" >&2
      echo "" >&2
      exit 1
    EOT
  }
  ...
  triggers = {
    ...
    protection_enabled = "true"  # Always true when this resource exists
  }
}
...
# ============================================================================
# NOTE: The Amplify branch (develop) is managed externally by Git/Amplify.
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      aws amplify update-branch \
        ...
        --branch-name ${var.amplify_dev_branch_name} \
