...
def handle_connect(event, connection_id):
    ...
    if connection_type in ['chat', 'booking'] and booking_id:
        try:
            store_connection(connection_id, booking_id, user_id=None, connection_type=connection_type)
        ...
        try:
            store_connection(connection_id, f"user_{user_id}", user_id=user_id, connection_type='notification')
...
def store_connection(connection_id: str, booking_id: str, user_id: str = None, connection_type: str = 'chat'):
    """
    Store WebSocket connection in DynamoDB.
    
    Args:
        connection_id: API Gateway connection ID
        booking_id: Booking ID this connection is for
    ...
    try:
        ...
        logger.info(f"Stored connection: {connection_id} for booking {booking_id}")
    except Exception as e:
...
def get_connection_ids_for_booking(booking_id: str, connection_type: str = 'chat'):
    ...
    try:
        ...
        logger.info(f"Found {len(connection_ids)} connections for booking {booking_id}")
        return connection_ids
    except Exception as e:
        logger.error(f"Failed to query connections from DynamoDB: {e}")
        return []
