...
current_api_endpoint = None  # Store current endpoint to compare
...
def lambda_handler(event, context):
    ...
    global apigw_management, current_api_endpoint
    ...
    # Initialize or recreate client if endpoint changed
    if api_endpoint and (apigw_management is None or current_api_endpoint != api_endpoint):
        apigw_management = boto3.client(
            'apigatewaymanagementapi',
            endpoint_url=api_endpoint,
            region_name=AWS_REGION
        )
        current_api_endpoint = api_endpoint
...
def handle_connect(event, connection_id):
    ...
    """
    # Extract query parameters - handle both None and dict cases
    ...
    logger.info(f"Connection: type={connection_type}, booking_id={booking_id}, connection_id={connection_id}, token={'present' if token else 'missing'}")
    ...
    # Token is required for booking, chat, and notification connections
    # Feed connections may not require authentication
    if connection_type in ['booking', 'chat', 'notification'] and not token:
        ...
        return {
            'statusCode': 401,
            'body': json.dumps({'error': 'token required for ' + connection_type + ' connections'})
        }
